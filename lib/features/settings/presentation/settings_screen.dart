  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: settingsState.isLoading
          ? const Center(child: CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color>(ColorConstants.neonViolet)))
                    ...
                    NeonCard(
                      ...
                      child: Column(
                        children: [
                          _buildSwitchRow(
                            'Push Notifications',
                            'Daily reminders & AI insights',
                            settingsState.value!.notificationsEnabled,
                            (val) => ref.read(settingsNotifierProvider.notifier).updateNotifications(val),
                          ),
                          const Divider(color: ColorConstants.cardSurface),
                          _buildSwitchRow(
                            'Haptic Feedback',
                            'Vibrations during timer & completion',
                            settingsState.value!.hapticsEnabled,
                            (val) => ref.read(settingsNotifierProvider.notifier).updateHaptics(val),
                          ),
                          const Divider(color: ColorConstants.cardSurface),
                          _buildSwitchRow(
                            'Real-time Cloud Sync',
                            'Encrypted state persistence',
                            settingsState.value!.cloudSyncEnabled,
                            (val) => ref.read(settingsNotifierProvider.notifier).updateCloudSync(val),
                          ),
                        ],
                      ),
                    ),
